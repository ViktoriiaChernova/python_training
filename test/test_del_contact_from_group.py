...
def test_del_contact_from_group(app, db):
    ...
    app.contact.delete_contact_from_group_by_id(c_id, g_id)
    contacts_in_group_after_deleting = db.get_contacts_in_group(Group(id=g_id))
    contacts_in_group_before_adding.remove(contact)
    assert sorted(contacts_in_group_before_adding, key=Contact.id_or_max) == sorted(contacts_in_group_after_deleting, key=Contact.id_or_max)
