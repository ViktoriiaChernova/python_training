...
def test_add_contact_to_group(app, db):
    ...
    contacts_not_in_group = db.get_contacts_not_in_group(Group(id=g_id))
    if len (contacts_not_in_group) == 0:
        ...
        app.contact.create(Contact(firstname="ghgfhg", middlename="jhjhjhj", lastname="jhjjgjhg", nickname="jghjjhggh",
                                   ...
                                   ayear="1989"))
    contact = random.choice(db.get_contacts_not_in_group(Group(id=g_id)))
    contacts_in_group_before_adding = db.get_contacts_in_group(Group(id=g_id))
    app.contact.add_contact_to_group_by_id(contact.id, g_id)
